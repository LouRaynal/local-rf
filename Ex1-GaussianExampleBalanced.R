...
resTestBayes <- table(classeTest, predTestingBayes)
resTestBayes
(sum(resTestBayes)-sum(diag(resTestBayes))) / nTest # Bayes missclassification error rate
(sum(diag(resTestBayes))) / nTest
...
predMKRF1 <- foreach(i=1:nTest, .combine="rbind", .packages = "matrixStats",
                     .export = c("KernelMultiGauss", "KernelRF")) %dopar% {
                       ...
                       modelMKRF <- forestLocalMultiple(y = factor(classe), x = x.trainNoised, obs = x.testNoised[i,],
                                                        multiMinNodeSize = minNodeSize, alpha = 1, ntree = 100, 
